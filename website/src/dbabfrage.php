    ...
    //$dbtest = true; // Decide if the database should store in testmode or in productivemode 
        ...
        // Dabei werden auch abgebrochene Wässerungsversuche gezählt, bei denen die 
        // Erde noch ausreichend feucht war

        $sql = "
        SELECT $tabelle.timestamp
        FROM $tabelle
        WHERE ((($tabelle.sensorname)='Banane') AND (($tabelle.logtype)='Giessung'))
        ORDER BY $tabelle.ID DESC LIMIT 1
        ";
