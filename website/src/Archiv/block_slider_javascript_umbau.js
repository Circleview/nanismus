(function(w, $, undefined) {
	...
	$.blockSlide = function(options, content, element) {
		...
		// The position of the "popup" from the top of the screen
		this.fromTop = 50;
	...
	$.blockSlide.defaults = {
		...
		// The increased height after the user clicks on it
		incheight : 360,       // durch mich geändert 320
	...
	$.blockSlide.prototype = {
		...
		start: function(options, content, element) {
			...
			if(!Modernizr.cssanimations) {
				...
				this.$element.find('[class^="bpage"]').css({'top' : '10px'});
		...
		append: function() {
			...
			for(var key in this.content) {
				...
				// von mir eingefügt start
                // http://de.selfhtml.org/javascript/objekte/window.htm#inner_width
                
                /*
                 * if(window.innerWidth < 340) { //nur bei kleinen screens soll die Position der Icons unterschiedlich sein.				
    				switch (x)
    				{
    				case 0:
    				     this.posArray[x] = 10;
				...
				this.$element.find('.bpage-'+x).css({
					...
					//'margin-top'  : this.topArray[x]+'px',  // von mir eingefügt
				...
				//if($(this).css('animation') == undefined) {
					...
					setTimeout(function showBack() {
						...
						that.parents('[class^="bpage"]').appendTo('body').css({
							...
							'margin-top'  : '10px', // von mir eingefügt
							'left' : '50%',
							'position' : 'fixed' 
						});
					...
					$('.pseudoblanket').fadeIn();  // Kann wohl auch ausgeschaltet werden, wenn
					// der schwarze Hintergrund ausgeschaltet ist
				//}
